  ...
  return (
    <div className="bg-white font-sans min-h-screen p-4">
        ...
        <div className="bg-[#290c52] p-4 rounded shadow-md text-white">
          ...
          <div className="grid grid-cols-2 gap-2 text-black mb-2">
            {mainLanguages.map((lang) => (
              <label key={lang} className="bg-white px-1 md:px-2 py-1 rounded flex items-center gap-1 w-12 md:w-auto">
                <input
                  type="radio"
                  name="mainLanguage"
          ...
          <div className="grid grid-cols-2 gap-2 text-black">
            {subLanguages.map((subLang) => {
              ...
              return (
                <label
                  key={subLang}
                  className={`${disabled ? "opacity-50 cursor-not-allowed" : ""} bg-white w-12 md:w-auto px-2 py-1 rounded flex items-center gap-1`}
                >
                  <input
                    type="radio"
                    name="subLanguage"
        ...
        <div className="bg-[#290c52] p-4 rounded shadow-md text-white">
          ...
          <div className="grid grid-cols-3 gap-2 mt-6">
            {durations.map((time) => (
              <label
                key={time}
                className={`px-2 py-1 rounded text-center font-medium mt-[-15] md:mt-0 cursor-pointer bg-white w-8 md:h-auto md:w-auto text-black border border-gray-400 ${duration === time ? "bg-blue-200" : ""}`}
              >
                <input
                  type="radio"
                  name="duration"
      ...
      <div className="flex flex-row min-h-screen bg-blue-200 bg-[url('/bg.jpg')]">
        {/* Sidebar Navigation */}
        <div className="w-32 bg-transparent text-white pt-14 space-y-17 text-xl md:text-4xl pl-10 flex flex-col">
        ...
        {/* Content Area */}
        <div className="bg-white p-6 shadow-md w-[60%] md:w-[70%] mx-auto mt-5">
          <h2 className="text-center font-bold italic mb-4 text-xl md:text-5xl">
            ...
            {sectionContent[selectedSection].map((item, idx) => {
              ...
              return (
                <li key={idx} className="flex items-center gap-4">
                  <span className="text-xl md:text-4xl">{lessonNum}</span>
                  ...
                  <span className="text-md md:text-4xl">{lessonTitle}</span>
                </li>
              );
            })}
          </ul>
